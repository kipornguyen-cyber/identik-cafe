@extends('layouts.app')
@section('title', 'Menu Premium -  Identik Cafe')
...
<section id="makanan" class="py-32 px-6 bg-gradient-to-b from-black to-gray-900">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            @foreach($makanan_utama as $item)
            <div class="menu-card group fade-in" style="animation-delay: {{ $loop->index * 0.1 }}s">
                <div class="overflow-hidden rounded-2xl mb-6">
                    <img src="{{ $item['image'] }}" alt="{{ $item['name'] }}"
                         class="group-hover:scale-110 transition-transform duration-700">
                </div>
                <div>
                    <h3 class="text-2xl font-bold text-white mb-4 group-hover:text-yellow-400 transition-colors">{{ $item['name'] }}</h3>
                    <div class="flex items-center justify-between mb-6">
                        <span class="text-3xl font-black text-yellow-400">{{ $item['price'] }}</span>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            ...
            <div class="menu-card group fade-in" style="animation-delay: {{ $loop->index * 0.1 }}s">
                ...
                <div>
                    <h3 class="text-xl font-bold text-white mb-3 group-hover:text-yellow-400 transition-colors">{{ $item['name'] }}</h3>
                    <div class="flex items-center justify-between mb-6">
                        <span class="text-2xl font-black text-yellow-400">{{ $item['price'] }}</span>
                    </div>
